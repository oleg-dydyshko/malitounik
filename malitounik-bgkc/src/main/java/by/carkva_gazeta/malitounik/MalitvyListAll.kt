...
fun getTrebnik(): SnapshotStateList<BogaslujbovyiaListData> {
    ...
    list.add(
        BogaslujbovyiaListData(
            "Чын малітвы за памерлых у велікодным часе", "bogashlugbovya/czyn_malitvy_za_pamierlych_vielikodny_czas.html", "БОГАСЛУЖБОВЫЯ ТЭКСТЫ -> ТРЭБНІК"
        )
    )
    list.add(
        BogaslujbovyiaListData(
            "Асьвячэньне новага дому", "bogashlugbovya/asviaczennie_novaha_domu.html", "БОГАСЛУЖБОВЫЯ ТЭКСТЫ -> ТРЭБНІК"
        )
    )
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.BAKLAVA) {
