...
import by.carkva_gazeta.malitounik.databinding.AdminBiblePageFragmentBinding
    ...
    private fun sendPostRequest(id: Int, spaw: String, sv: Int) {
        activity?.let { fragmentActivity ->
            if (Settings.isNetworkAvailable(fragmentActivity)) {
                bibleJob = CoroutineScope(Dispatchers.Main).launch {
                    ...
                    Malitounik.referens.child("/chytanne/Semucha/biblias$id.txt").getFile(localFile).addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            ...
                            Toast.makeText(fragmentActivity, getString(R.string.error), Toast.LENGTH_SHORT).show()
                        }
                    }.await()
                    saveLogFile()
                    Malitounik.referens.child("/chytanne/Semucha/biblias$id.txt").putFile(Uri.fromFile(localFile)).addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            Toast.makeText(fragmentActivity, getString(R.string.save), Toast.LENGTH_SHORT).show()
                        } else {
                            Toast.makeText(fragmentActivity, getString(R.string.error), Toast.LENGTH_SHORT).show()
                        }
                    }.await()
                ...
                activity?.let {
                    Toast.makeText(it, getString(R.string.no_internet), Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
            ...
            urlJob = CoroutineScope(Dispatchers.Main).launch {
                activity?.let { fragmentActivity ->
                    ...
                    try {
                        ...
                        context?.let {
                            Toast.makeText(fragmentActivity, getString(R.string.error_ch2), Toast.LENGTH_SHORT).show()
                        }
                    }
