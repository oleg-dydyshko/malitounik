...
import by.carkva_gazeta.malitounik.databinding.PiasochnicaNetFileExplorerBinding
...
class PiasochnicaNetFileExplorer : BaseActivity(), DialogContextMenu.DialogContextMenuListener, DialogPasochnicaFileName.DialogPasochnicaFileNameListener, DialogDelite.DialogDeliteListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.content.selector = ContextCompat.getDrawable(this, R.drawable.selector_default)
    ...
    private fun setTollbarTheme() {
        ...
        binding.titleToolbar.text = getString(R.string.vybrac_file)
    }
        ...
        if (Settings.isNetworkAvailable(this)) {
            netFileJob = CoroutineScope(Dispatchers.Main).launch {
                ...
                try {
                    ...
                    Toast.makeText(this@PiasochnicaNetFileExplorer, getString(R.string.error_ch2), Toast.LENGTH_SHORT).show()
                }
    ...
    private suspend fun saveLogFile(count: Int = 0) {
        ...
        Malitounik.referens.child("/admin/log.txt").putFile(Uri.fromFile(logFile)).addOnFailureListener {
            Toast.makeText(this@PiasochnicaNetFileExplorer, getString(R.string.error), Toast.LENGTH_SHORT).show()
            error = true
        }.await()
        if (error && count < 3) {
            saveLogFile(count + 1)
