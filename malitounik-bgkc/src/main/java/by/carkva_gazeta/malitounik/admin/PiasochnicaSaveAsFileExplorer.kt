...
import by.carkva_gazeta.malitounik.databinding.AdminDialigSaveAsBinding
import by.carkva_gazeta.malitounik.databinding.AdminSimpleListItemBinding
...
class PiasochnicaSaveAsFileExplorer : BaseActivity(), DialogPasochnicaMkDir.DialogPasochnicaMkDirListener {
        ...
        override fun afterTextChanged(s: Editable?) {
            if (editch) {
                ...
                if (newEdit != edit) {
                    editPosition -= 1
                    edit = newEdit
                    Toast.makeText(this@PiasochnicaSaveAsFileExplorer, getString(R.string.admin_cyrylic_no_support), Toast.LENGTH_SHORT).show()
                }
    ...
    private fun setTollbarTheme() {
        ...
        binding.titleToolbar.text = getString(R.string.save_as_up)
        ...
        if (Settings.isNetworkAvailable(this)) {
            saveAsFileJob = CoroutineScope(Dispatchers.Main).launch {
                ...
                try {
                    ...
                    Toast.makeText(this@PiasochnicaSaveAsFileExplorer, getString(R.string.error_ch2), Toast.LENGTH_SHORT).show()
                }
    ...
    private inner class TitleListAdaprer(private val mContext: Activity) : ArrayAdapter<MyNetFile>(mContext, R.layout.admin_simple_list_item, fileList) {
        override fun getView(position: Int, mView: View?, parent: ViewGroup): View {
            ...
            if (title == "admin" || title == "bogashlugbovya" || title == "parafii_bgkc" || title == "prynagodnyia" || title == "pesny") {
                viewHolder.text.typeface = createFont(Typeface.BOLD)
            } else {
                viewHolder.text.typeface = createFont(Typeface.NORMAL)
            }
            viewHolder.text.background = ContextCompat.getDrawable(mContext, R.color.colorWhite)
        ...
        fun createFont(style: Int): Typeface? {
            return when (style) {
                Typeface.BOLD -> ResourcesCompat.getFont(mContext, R.font.roboto_condensed_bold)
                Typeface.ITALIC -> ResourcesCompat.getFont(mContext, R.font.roboto_condensed_italic)
                Typeface.BOLD_ITALIC -> ResourcesCompat.getFont(mContext, R.font.roboto_condensed_bold_italic)
                else -> ResourcesCompat.getFont(mContext, R.font.roboto_condensed_regular)
            }
        }
    }
