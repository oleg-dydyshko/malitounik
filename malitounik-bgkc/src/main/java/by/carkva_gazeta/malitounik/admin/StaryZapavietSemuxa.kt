...
import by.carkva_gazeta.malitounik.databinding.AdminBibleBinding
...
class StaryZapavietSemuxa : BaseActivity(), DialogBibleRazdel.DialogBibleRazdelListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TabLayoutMediator(binding.tabLayout, binding.pager, false) { tab, position ->
            tab.text = if (kniga == 18) resources.getString(R.string.psalom) + " " + (position + 1) else resources.getString(R.string.razdzel) + " " + (position + 1)
        }.attach()
