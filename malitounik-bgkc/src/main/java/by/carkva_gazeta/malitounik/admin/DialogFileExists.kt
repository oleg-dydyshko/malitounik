...
class DialogFileExists : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        activity?.let {
            ...
            binding.title.text = resources.getString(R.string.file_exists)
            binding.content.text = getString(R.string.file_exists_opis, fileName)
            val saveAs = arguments?.getBoolean("saveAs") != false
            builder.setPositiveButton(resources.getText(R.string.file_perazapisac)) { _: DialogInterface, _: Int -> mListener?.fileExists(dir, oldFileName, fileName, saveAs) }
