...
class DialogPasochnicaMkDir : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        activity?.let {
            ...
            binding.title.text = getString(R.string.set_file_dir)
                ...
                if (Settings.isNetworkAvailable(fragmentActivity)) {
                    CoroutineScope(Dispatchers.Main).launch {
                        try {
                            ...
                            activity?.let {
                                Toast.makeText(it, getString(R.string.error_ch2), Toast.LENGTH_SHORT).show()
                            }
                        }
                        mListener?.setDir("/$dir/$dirName/", oldName, newName)
                    }
                } else {
                    Toast.makeText(fragmentActivity, getString(R.string.no_internet), Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
