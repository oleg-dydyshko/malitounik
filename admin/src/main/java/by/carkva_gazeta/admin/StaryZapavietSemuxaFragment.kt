        ...
        activity?.let { fragmentActivity ->
            if (Settings.isNetworkAvailable(fragmentActivity)) {
                bibleJob = CoroutineScope(Dispatchers.Main).launch {
                    ...
                    MainActivity.referens.child("/chytanne/Semucha/biblias$id.txt").getFile(localFile).addOnCompleteListener { task ->
                    ...
                    saveLogFile()
                    MainActivity.referens.child("/chytanne/Semucha/biblias$id.txt").putFile(Uri.fromFile(localFile)).addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            Toast.makeText(fragmentActivity, getString(by.carkva_gazeta.malitounik2.R.string.save), Toast.LENGTH_SHORT).show()
                        } else {
                            Toast.makeText(fragmentActivity, getString(by.carkva_gazeta.malitounik2.R.string.error), Toast.LENGTH_SHORT).show()
                        }
                    }.await()
        ...
        if (Settings.isNetworkAvailable(requireActivity())) {
            ...
            urlJob = CoroutineScope(Dispatchers.Main).launch {
                activity?.let { fragmentActivity ->
                    ...
                    try {
                        val localFile = File("${fragmentActivity.filesDir}/cache/cache.txt")
                        MainActivity.referens.child(url).getFile(localFile).addOnCompleteListener { task ->
                            if (task.isSuccessful) {
